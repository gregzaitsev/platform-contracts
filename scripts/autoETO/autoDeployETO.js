...
module.exports = async function deploy() {
  ...
  const CONFIG = getConfig(web3, autoConfig.Network, []);
  const DEPLOYER = autoConfig.Deployer;
  const Universe = artifacts.require(CONFIG.artifacts.UNIVERSE);
  const universe = await Universe.at(autoConfig.Universe);
  const Q18 = CONFIG.Q18;

  const contents = fs.readFileSync(autoConfig.ETODefinitionFile);
